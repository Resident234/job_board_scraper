...
public static class HttpClientFactory
{
    ...
    public static ProxyRotator? CreateProxyRotator()
    {
        ...
        {
            System.Console.WriteLine("[HttpClientFactory] Список прокси пуст. Загрузка из публичных источников...");
            ...
            try
            {
                ...
                task1.Wait();
                System.Console.WriteLine($"[HttpClientFactory] ProxyScrape: загружено {provider.GetProxies().Count} прокси");
                
                var task2 = provider.LoadFromGeoNodeAsync(50);
                task2.Wait();
                System.Console.WriteLine($"[HttpClientFactory] GeoNode: загружено {provider.GetProxies().Count} прокси");
                
                proxyList = provider.GetProxies();
                
                if (proxyList.Count == 0)
                {
                    System.Console.WriteLine("[HttpClientFactory] ⚠️ Не удалось загрузить прокси из публичных источников");
                    return null;
                }
                
                System.Console.WriteLine($"[HttpClientFactory] ✓ Загружено {proxyList.Count} прокси");
            }
            catch (Exception ex)
            {
                System.Console.WriteLine($"[HttpClientFactory] ⚠️ Ошибка загрузки прокси: {ex.Message}");
                return null;
            }
        }
    ...
    public static async Task<ProxyProvider> CreateProxyProviderAsync()
    {
        ...
        {
            System.Console.WriteLine("[HttpClientFactory] Загрузка прокси из публичных источников...");
            await provider.LoadFromProxyScrapeAsync();
            await provider.LoadFromGeoNodeAsync(50);
            
            var count = provider.GetProxies().Count;
            System.Console.WriteLine($"[HttpClientFactory] Загружено {count} прокси");
        }
