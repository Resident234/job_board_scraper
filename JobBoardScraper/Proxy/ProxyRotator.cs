...
public sealed class ProxyRotator
{
    ...
    public ProxyRotator(List<string> proxyUrls)
    {
        ...
        if (_proxies.Count == 0) _enabled = false;
    }
    ...
    {
        if (!_enabled || _proxies.Count == 0) return null;
        lock (_lock)
        {
    ...
    {
        if (!_enabled || _proxies.Count == 0) return null;
        lock (_lock) { return _proxies[_currentIndex]; }
    }
    ...
    {
        if (!_enabled) return "Proxy rotation disabled";
        lock (_lock) { return $"Proxy {_currentIndex + 1}/{_proxies.Count}"; }
    }
}
